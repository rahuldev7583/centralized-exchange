...
const WS_URL: string | undefined = process.env.NEXT_PUBLIC_WS_URL;
...
const WS_ENDPOINT: string = WS_URL;
...
class WsClient {
    ...
    connect() {
        ...
        const url = token ? `${WS_ENDPOINT}?token=${encodeURIComponent(token)}` : WS_ENDPOINT;
